-- Checkout-only workers; preserve all historical sales and payment methods.
begin;

-- Self-update previously allowed workers to promote their own role.
drop policy if exists "profiles update self" on public.profiles;
alter policy "profiles read members" on public.profiles using (id = auth.uid() or public.is_owner());
alter policy "sales read" on public.sales using (public.is_owner());
alter policy "customers members read" on public.customers using (public.is_owner());
alter policy "payments members read" on public.payments using (public.is_owner());
alter policy "inventory transactions members read" on public.inventory_transactions using (public.is_owner());

alter table public.payments drop constraint if exists payments_payment_method_check;
alter table public.payments add constraint payments_payment_method_check
  check (payment_method in ('cash', 'mobile_money', 'card', 'transfer'));
...
begin
  ...
  if p_payment_method is null or p_payment_method not in ('cash', 'mobile_money') then
...
declare
  ...
  new_status text;
begin
  if not public.is_owner() then raise exception 'Only the owner can perform this action'; end if;
  if p_payment_method not in ('cash', 'mobile_money', 'card', 'transfer') then
    raise exception 'Invalid payment method';
  end if;
