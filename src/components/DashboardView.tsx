...
export function DashboardView({ onNavigate }: Props) {
  ...
  const data = summarizeStore(products, sales, payments)
  const max = Math.max(1, ...data.series.map((day) => day.total))
  return <div className="view dashboard-view">
    <header className="view-header">
      <div><p className="eyebrow">Your store at a glance</p><h1>Overview</h1><p className="page-description">Sales, stock, and balances. One clear picture.</p></div>
      <button className="primary-btn" onClick={() => onNavigate('sell')}><Plus size={18} /> New sale</button>
    </header>
    <div className="retail-metrics">
      <button className="retail-metric metric-featured" onClick={() => onNavigate('sales')}><span>Sales today <ArrowUpRight size={18}/></span><strong>{money(data.salesToday)}</strong><small>{data.countToday} completed sales · including tax</small></button>
      <button className="retail-metric" onClick={() => onNavigate('sales')}><span>{isOwner ? 'Gross profit today' : 'Payments today'} <ArrowUpRight size={18}/></span><strong>{money(isOwner ? data.profitToday : data.collected)}</strong><small>{isOwner ? 'Before operating expenses' : 'Money received today'}</small></button>
      <button className="retail-metric" onClick={() => onNavigate('credit')}><span>Customer balances <ArrowUpRight size={18}/></span><strong>{money(data.balance)}</strong><small>Outstanding across all sales</small></button>
      <button className="retail-metric" onClick={() => onNavigate('stock')}><span>Needs restocking <ArrowUpRight size={18}/></span><strong>{data.low.length}<em>products</em></strong><small>{data.low.filter((p) => p.stock === 0).length} currently out of stock</small></button>
    </div>
    {offlinePending > 0 && <p className="offline-banner">{offlinePending} offline sales waiting to sync.</p>}
    {!products.length && <section className="setup-strip"><span className="setup-icon"><Package size={24}/></span><div><h2>Start with your first product</h2><p>Add your catalog and opening stock to begin selling.</p></div><button className="ghost-btn" onClick={() => onNavigate('stock')}>Open products <ArrowUpRight size={17}/></button></section>}
